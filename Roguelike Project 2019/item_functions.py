...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        results.append({'consumed': True, 'target': target, 
                        'message': Message('A lightning bolt strikes the {0} dealing {1} damage!'.format(target.name, damage), libtcod.cyan)})
...
def cast_fireball(*args, **kwargs):
    ...
    if not libtcod.map_is_in_fov(fov_map, target_x, target_y):
        results.append({'consumed': False, 'message': Message('You cannot target a tile you cannot see.', libtcod.yellow)})
        return results
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            confused.ai = ConfusedMonster(entity.ai, 10)
